...
void uvgrtp::reception_flow::call_ecn_handlers(uint32_t key, uvgrtp::frame::rtp_frame *frame, int ecn_bit) {
    auto& ecn_handler = packet_handlers_[key].ecn;
    if (ecn_handler.handler != nullptr && ecn_handler.arg != nullptr)
        ecn_handler.handler(ecn_handler.arg, frame, ecn_bit);
}
        ...
        while (ring_read_index_ != last_ring_write_index_)
        {
            ...
            if (ring_buffer_[ring_read_index_].read > 0)
            {
                ...
                for (auto& handler : packet_handlers_) {
                    ...
                    call_ecn_handlers(handler.first, frame, ring_buffer_[ring_read_index_].ecn_bit);
